...
//  Copyright (c) 2015 Ricardo Vera. All rights reserved.
//
...
- (IBAction)accionAlpha:(id)sender {
    ...
    self.lblBlink.text = [NSString stringWithFormat:@"Parpadea: %.f",alpha];
    ...
    if(self.sliderAlpha.value){
        myTimer = [NSTimer scheduledTimerWithTimeInterval:(self.sliderAlpha.value) target:self selector:@selector(blink) userInfo:nil repeats:YES];
    }
